...
if ($status=="tambah") { $id=0; }
	$query="SELECT id_divisi, divisi FROM divisi WHERE id_divisi='$id'" ;
	$hquery=$koneksi->query($query);
	$dataquery=mysqli_fetch_array($hquery);
?>
<h1 style="font-size:40px; text-align: center;"><?php if($status=="edit") { echo "Ubah"; } elseif ($status=="tambah") { echo "Tambah"; } ?> Divisi</h1>
<br>
<div class="container-fluid">
<form action="aksi_divisi.php?act=<?php echo"$status"; ?>" method="post" enctype="multipart/form-data" id="form1" class="card card-body col-4 mx-auto">
                            <div class="mb-3">
                            <input type="hidden" name="id" value="<?php echo"$dataquery[id_divisi]"; ?>" />
                            <label for="seleksi" class="form-label">Nama Divisi</label>
                            <input type="text" class="form-control required" maxlength="50" name="divisi" value="<?php echo @$dataquery['divisi']; ?>"/>
                            </div>
